...
"""
with open("absent.txt", 'wt') as file:
    while True:
        name=input("Name of absentee")
...
with open("passwd", 'rt') as file:
    for line in file:
        #print(line.split(":"))
        userdict[line.split(":")[0]]=line.split(":")[2]
...
with open(filename,'rt') as file:
    ...
    for line in file:
        ...
        for x in line.split():
            #print(x)
            wordlist.append(x)
    print("no of lines={}".format(no_line))
    print("no of words= {}".format(len(wordlist)))
    print("no of unique word={}".format(len(set(wordlist))))
    print("no of character ={}".format(no_char))
...
userimg=Image.open("sample.jpg")
userimg=userimg.convert("LA")
userimg.save("userimg.png")
w,h=userimg.size

mw=w/2
mh=h/2

userimg=userimg.crop((mw-80,mh-102,mw+80,mh+102))

userimg=userimg.transpose(Image.ROTATE_90)

userimg.thumbnail((75,75))

print(userimg.wdith,userimg.height)
...
with open("passwd",'rt') as file:
    ...
    with open("result.csv",'wt') as outfile:
